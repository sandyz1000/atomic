mod common;
use common::{col_as_string, make_employees_batch, make_kv_batch, make_salaries_batch, total_rows};
...
async fn test_inner_join() {
    ...
    ctx.register_batches("emp", vec![make_employees_batch()])
        .unwrap();
    ctx.register_batches("sal", vec![make_salaries_batch()])
        .unwrap();
    ...
    assert_eq!(
        total_rows(&batches),
        2,
        "inner join: only Alice and Bob have salaries"
    );
...
async fn test_cross_join() {
    ...
    ctx.register_batches("a", vec![make_kv_batch(&[1, 2], &[10, 20])])
        .unwrap();
    ctx.register_batches("b", vec![make_kv_batch(&[3, 4], &[30, 40])])
        .unwrap();
...
async fn test_join_no_matches() {
    ...
    ctx.register_batches("a", vec![make_kv_batch(&[1, 2], &[10, 20])])
        .unwrap();
    ctx.register_batches("b", vec![make_kv_batch(&[99, 100], &[90, 100])])
        .unwrap();
