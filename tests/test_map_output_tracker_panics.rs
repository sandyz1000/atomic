...
//!
//!   B2 (line 302): `self.map_output_uris.get_mut(&shuffle_id).unwrap()[map_id]`
...
//!
//!   B4 (unregister path): `self.map_output_uris.get_mut(&shuffle_id).unwrap()…`
...
fn test_register_map_ok() {
    ...
    assert!(t.map_output_uris.contains_key(&1));
}
...
fn test_unregister_oob() {
    ...
    let uris = t.map_output_uris.get(&7).unwrap();
...
fn test_register_shuffle_idempotent() {
    ...
    assert!(t.map_output_uris.contains_key(&10));
    assert_eq!(t.map_output_uris.get(&10).unwrap().len(), 4);
}
