...
fn main() -> Result<()> {
    ...
    let files = context
        ...
        .flat_map(Fn!(
            |iter: Vec<((i32, String, i64), (i64, f64))>| 
                Box::new(iter.into_iter()) as Box<dyn Iterator<Item = _>>
        ));
    let sum = files.reduce_by_key(Fn!(|((vl, cl), (vr, cr))| (vl + vr, cl + cr)), 1);
    let avg = sum.map(Fn!(|(k, (v, c))| (k, v as f64 / c)));
    let res: Vec<_> = avg.collect().unwrap();
...
fn read(file: PathBuf) -> Vec<((i32, String, i64), (i64, f64))> {
    ...
    let iter = (0..metadata.num_row_groups()).flat_map(move |i| {
        ...
        chunks.into_iter().flat_map(move |(begin, end)| {
            ...
            let mut first: Vec<_> = vec![Default::default(); end - begin];
            let mut second: Vec<_> = vec![Default::default(); end - begin];
            let mut time: Vec<_> = vec![Default::default(); end - begin];
            let mut bytes: Vec<_> = vec![Default::default(); end - begin];
