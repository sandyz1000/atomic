...
impl ReaderConfiguration<Vec<u8>> for LocalFsReaderConfig {
        ...
        U: Data,
    {
        ...
        let read_files = serde_closure::Fn!(
            |_part: usize, readers: Box<dyn Iterator<Item = BytesReader>>| 
            {
                Box::new(readers.into_iter().map(|reader| reader.into_iter()).flatten()) as Box<dyn Iterator<Item = _>>
            }
        );
...
impl ReaderConfiguration<PathBuf> for LocalFsReaderConfig {
        ...
        U: Data,
    {
        ...
        let read_files = serde_closure::Fn!(
            ...
            {
                Box::new(readers.map(|reader| reader.into_iter()).flatten()) as Box<dyn Iterator<Item = _>>
            }
        );
        let files_per_executor = Arc::new(
            MapPartitionsRdd::new(Arc::new(reader), read_files).pin(),
...
impl<T: Data> LocalFsReader<T> {
    ...
    /// This function should be called once per host to come with the parallel workload.
