...
- **Locality-aware cache scheduling**: tracking which workers hold cached partitions is deferred. The orphan `CacheTracker`/`BoundedMemoryCache` Vega port was removed; the live `PartitionStore` handles local caching without it, and `preferred_locations` returns empty for cached partitions on workers.
