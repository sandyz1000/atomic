...
impl MapOutputTracker {
    ...
    fn server(&self) {
        type ServerURIsMap = DashMap<usize, Vec<Option<String>>>;
        if !self.is_master {
            return;
        }
    ...
    pub fn register_shuffle(&self, shuffle_id: usize, num_maps: usize) {
        ...
        if let Some(_) = self.server_uris.get(&shuffle_id) {
            // TODO: error handling
    ...
    pub fn register_map_output(&self, shuffle_id: usize, map_id: usize, server_uri: String) {
        ...
        let mut sui = self.server_uris.get_mut(&shuffle_id).unwrap();
        sui[map_id] = Some(server_uri);
    }
    ...
    pub async fn get_server_uris(&self, shuffle_id: usize) -> Result<Vec<String>> {
        ...
        if self
            .server_uris
            .get(&shuffle_id)
            .map(|some| some.iter().find_map(|x| x.clone()).next())
            .flatten()
            .is_none()
        {
