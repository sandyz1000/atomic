...
pub(crate) trait NativeScheduler: Send + Sync {
        ...
        L: JobListener,
    {
        ...
        if stage == jt.final_stage {
            ...
            for (id_in_job, (id, part)) in jt
                ...
                .enumerate()
            {
                let locs: Vec<Ipv4Addr> =
                    self.get_preferred_locs(jt.final_rdd.get_rdd_base(), *part);
        ...
        R: Clone + Debug + Send + Sync + 'static,
    {
        ...
        let this = self.clone();
        let _lock = this.scheduler_lock.lock();
        ...
        L: JobListener,
    {
        ...
        if allow_local {
            if let Some(result) = Self::local_execution(jt.clone())? {
                return Ok(result);
            }
        }
        ...
        while num_finished != jt.num_output_parts {
            // Look for event after stage submit
...
// Implementing the SchedulerMixin trait for all types which implements NativeScheduler
impl<T> SchedulerMixin for T where T: NativeScheduler {}
