...
impl Executor {
    ...
    /// TODO: Remove unnecessary Arc from code.
            ...
            let res: Result<Signal> = tokio::task::spawn(async move {
                ...
                match rcv_main.try_recv() {
                    ...
                    Ok(Signal::ShutDownGracefully) => {
                        log::info!("shutting down executor @{} gracefully", _exec.port);
                        return Ok(Signal::ShutDownGracefully);
                    }
                    _ => {}
                }
                log::debug!("received new task @{} executor", _exec.port);
                let message = {
    ...
    async fn signal_handler(self: Arc<Self>, send_child: Sender<Signal>) -> Result<Signal> {
        ...
        // TODO: Fix the listener this code
...
mod tests {
    ...
    async fn send_task() -> Result<()> {
        fn test(client_rcv: Receiver<ComputeResult>, port: Port) -> Result<()> {
            // Mock data:
            let func = serde_closure::Fn!(move |(_task_context, iter): (
                TaskContext,
