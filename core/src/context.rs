...
impl Context {
    ...
    fn init_distributed_driver() -> Result<Arc<Self>> {
        ...
        let binary_path: PathBuf = std::env::current_exe().map_err(|_| Error::CurrentBinaryPath)?;
        ...
        for address in &hosts::Hosts::get()?.slaves {
            ...
            // TODO: Have better file transfer and binary to remote worker
            // Use tool like fabric
        ...
        F: Fn(Box<dyn Iterator<Item = T>>) -> U,
    {
        ...
        // NOTE: This Arc to closure make sense, since this will be 
        // serialized and transfer to different thread
        let func = Arc::new(cl);
