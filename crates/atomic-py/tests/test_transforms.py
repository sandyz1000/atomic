...
# ── P4.2: Broadcast variables ─────────────────────────────────────────────────


def test_broadcast_dict(ctx):
    """Broadcast a dict and read it back via value()."""
    data = {"threshold": 10, "label": "test"}
    bv = ctx.broadcast(data)
    result = bv.value()
    assert result["threshold"] == 10
    assert result["label"] == "test"


def test_broadcast_list_value(ctx):
    """Broadcast a list and verify value() returns the list."""
    lst = [1, 2, 3, 4, 5]
    bv = ctx.broadcast(lst)
    assert bv.value() == lst


def test_broadcast_multiple_reads(ctx):
    """Call value() twice; both results must be equal."""
    bv = ctx.broadcast({"key": 42})
    first = bv.value()
    second = bv.value()
    assert first == second


def test_broadcast_repr(ctx):
    """BroadcastVar repr contains 'BroadcastVar' and byte size."""
    bv = ctx.broadcast({"x": 1})
    r = repr(bv)
    assert "BroadcastVar" in r
    assert "bytes" in r


def test_broadcast_used_in_closure(ctx):
    """Value can be captured in a closure passed to map()."""
    multiplier = ctx.broadcast(3)
    result = ctx.parallelize([1, 2, 3]).map(lambda x: x * multiplier.value()).collect()
...
def test_accumulator_int_sum(ctx):
    """Add 1 per element; final value equals element count."""
    acc = ctx.accumulator(0)
    data = list(range(10))
    ctx.parallelize(data).for_each(lambda _: acc.add(1))
    assert acc.value() == 10


def test_accumulator_float(ctx):
    """Add 0.5 per element; final value is len/2."""
    acc = ctx.accumulator(0.0)
    data = [1, 2, 3, 4]
    ctx.parallelize(data).for_each(lambda _: acc.add(0.5))
    assert abs(acc.value() - 2.0) < 1e-9


def test_accumulator_reset(ctx):
    """Add some values then reset; value returns to initial."""
    acc = ctx.accumulator(0)
    ctx.parallelize([1, 2, 3]).for_each(lambda _: acc.add(1))
    assert acc.value() == 3
    acc.reset()
    assert acc.value() == 0


def test_accumulator_list_append(ctx):
    """Start with an empty list; appending single ints grows the list."""
    acc = ctx.accumulator([])
    ctx.parallelize([10, 20, 30]).for_each(lambda x: acc.add(x))
    result = acc.value()
    assert len(result) == 3
    assert sorted(result) == [10, 20, 30]


def test_accumulator_empty_rdd(ctx):
    """for_each on an empty RDD; value stays at zero."""
    acc = ctx.accumulator(0)
    ctx.parallelize([]).for_each(lambda _: acc.add(1))
    assert acc.value() == 0


def test_accumulator_repr(ctx):
    """Accumulator repr contains 'Accumulator' and current value."""
    acc = ctx.accumulator(7)
    r = repr(acc)
    assert "Accumulator" in r


def test_accumulator_int_element_sum(ctx):
    """Sum elements directly using accumulator."""
    acc = ctx.accumulator(0)
    ctx.parallelize([5, 10, 15]).for_each(lambda x: acc.add(x))
    assert acc.value() == 30
