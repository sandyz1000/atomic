...
- **SRP module decomposition (7 files)**: applied the same `rdd/typed/` split pattern to six more large files, each broken into focused single-responsibility submodules:
  - `atomic-data/src/distributed.rs` (709 L) → `distributed/{mod,envelope,capabilities,transport,wire}.rs`
  - `atomic-compute/src/env.rs` (790 L) → `env/{mod,log_config,runtime,builder}.rs`; `builder.rs` adds `ConfigKey` enum (typed keys for `ConfigBuilder::set`, derives `serde::Deserialize` for config-file loading)
  - `atomic-scheduler/src/distributed.rs` (1596 L) → `distributed/{mod,worker_pool,transport,job_runner,shuffle_stage,cache_dispatch}.rs`
  - `atomic-compute/src/context.rs` (1194 L) → `context/{mod,broadcast,io,job_runner,worker}.rs`
