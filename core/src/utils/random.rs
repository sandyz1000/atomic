...
impl BernoulliCellSampler {
    pub fn new(lb: f64, ub: f64, complement: bool) -> BernoulliCellSampler {
        ...
        let ans = format!("Lower bound {} must be <= upper bound {}", lb, ub);
        assert!(lb <= (ub + ROUNDING_EPSILON), "{}", ans);
        let ans = format!("Lower bound {} must be >= 0.0", lb);
        assert!(lb >= (0.0 - ROUNDING_EPSILON), "{}", ans);
        let ans = format!("Upper bound {} must be <= 1.0", ub);
        assert!(ub <= (1.0 + ROUNDING_EPSILON), "{}", ans);
