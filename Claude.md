...
- **Lineage-based fault recovery (executor loss)**: a reduce task that cannot fetch a map output from a dead worker surfaces `BaseError::FetchFailed { shuffle_id, map_id, server_uri }` (from `ShuffleError::FetchFailed`); `LocalScheduler::run_task` classifies it as `TastEndReason::FetchFailed`, and the already-ported `on_event_failure` invalidates that output (`remove_output_loc_from_stage` + `MapOutputTracker::unregister_map_output`) and marks the producing map stage failed so `submit_missing_tasks` recomputes only the lost partition (`output_locs[p].is_empty()`). `DistributedScheduler::remove_worker` proactively calls `MapOutputTracker::unregister_outputs_on_host` on heartbeat-detected death. The reduce `ResultTask::run` now propagates compute errors via `?` instead of panicking. Tests: `shuffle::error::fetch_failed_preserved`, `shuffle::map_output::host_outputs_invalidated`, `test_worker_fault_tolerance::removed_worker_clears_outputs`.
- **`rdd/typed.rs` split into `rdd/typed/`**: the 2.6k-line file is grouped by responsibility — `mod.rs` (struct + `RddBase`/`Rdd`/`RddExt`), `persist.rs`, `actions.rs`, `transforms.rs`, `pair_ops.rs`, `joins.rs`, `sort.rs`, `task_api.rs`. Behaviour-preserving; submodules access private fields as descendants of `typed`.
- **Removed the orphan Vega cache subtree**: `CacheTracker`, `BoundedMemoryCache`, and `KeySpace` (a never-instantiated local-cache-tracking port whose eviction was a `todo!()` in Vega itself) were deleted. The live LRU cache is `PartitionStore` (`cache/mod.rs`); `get_cache_locs` is backed by `Mutators`.
...
- **Distributed RDD caching + locality scheduling**: implemented. A distributed `.cache()`/`persist()` appends a `TaskAction::Cache { rdd_id }` to the staged pipeline; the worker stores the partition bytes in a process-global `WORKER_PARTITION_CACHE` (LRU-bounded) and reports `(rdd_id, partition)` in `TaskResultEnvelope.cached_partitions`. The driver registers these into `Mutators.cache_locs`. On a later action, `DistributedScheduler::plan_cache_dispatch` detects the cached boundary and dispatches `TaskEnvelope.cache_source` read tasks **pinned to the holding worker** (`pick_preferred_worker`) instead of recomputing. Worker death invalidates that host's `cache_locs` (`remove_worker`) so the next job recomputes; `unpersist()` clears `cache_locs[rdd_id]`. Residual edge: a serve read that misses because the holder *evicted* the partition under LRU pressure (rather than dying) fails the job rather than transparently recomputing — re-running the job recomputes it; true per-task recompute fallback is a follow-up. Local-mode caching is unchanged (`PartitionStore` + `CachedRdd`).
